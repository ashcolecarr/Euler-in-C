...
#define ADJACENT    4
#define GRID_SIZE   sizeof(grid) / sizeof(grid[0])
...
int main()
{
    ...
    {
        for(i = 0; i < GRID_SIZE - ADJACENT; ++i)
        {
    ...
    {
        for(j = 0; j < GRID_SIZE - ADJACENT; ++j)
        {
    ...
    int k = j;
    int l = i;
    while(i < GRID_SIZE - ADJACENT && j < GRID_SIZE)
    {
        while(0 <= (j + 1) - ADJACENT && i + ADJACENT < GRID_SIZE)
        {
        ...
        /* Move to the next diagonal line. */
        if(k + 1 < GRID_SIZE)
        {
            j = ++k;
            i = 0;
    ...
    // Scan diagonally bottom left to top right
    i = 0; j = GRID_SIZE - ADJACENT;
    k = j;
    l = i;
    while(i < GRID_SIZE - ADJACENT && 0 < j)
    {
        while(j + ADJACENT < GRID_SIZE && i + ADJACENT < GRID_SIZE)
        {
            tempProduct = grid[i][j] * grid[i + 1][j + 1] * grid[i + 2][j + 2] * grid[i + 3][j + 3];
    ...
    printf("The largest product is %.0f.\n", finalProduct);
